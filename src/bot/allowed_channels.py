...
#
# Features:
#   - Prefix + slash allowlist commands:
...
#   - On startup, the bot scans pinned config messages across guilds
#     once and restores the allowlist, so you don't have to run
#     /allow_here every reboot (after initial setup).
...
def init_allowed() -> None:
    """Called from the main bot to initialise in-memory state."""
    _load_allowed()
...
def slash_allowed_check(interaction: discord.Interaction) -> bool:
    """
    Predicate used with @app_commands.check(slash_allowed_check).
    Raises CheckFailure if the channel is not allowed.
    """
...
def _encode_config_payload(guild: discord.Guild) -> str:
    """
    Build the text body for the pinned config message.
    """
    data = {
        "guild_id": guild.id,
...
def register_allowed_admin_commands(bot: commands.Bot) -> None:
    ...
    try:
        ...
        # Non-fatal if we can't schedule the task.
        pass
    ...
    @bot.command(
        name="allow_here",
        help="(Admin) Allow this channel for bot commands in this server.",
    )
    async def allow_here_prefix(ctx: commands.Context):
        ...
        await ctx.reply(
            f"✅ Allowed channel set: <#{cid}>",
            mention_author=False,
        )

    @bot.command(
        name="clear_allowed",
        help="(Admin) Clear all allowed-channel restrictions for this server.",
    )
    async def clear_allowed_prefix(ctx: commands.Context):
        ...
        await ctx.reply(
            "✅ Cleared restrictions — bot allowed in all channels here.",
            mention_author=False,
        )

    @bot.command(
        name="list_allowed",
        help="(Admin) List allowed channels for this server.",
    )
    async def list_allowed_prefix(ctx: commands.Context):
        ...
        if not chans:
            return await ctx.reply(
                "No restrictions set — bot allowed in all channels.",
                mention_author=False,
            )

        await ctx.reply(
            "Allowed channels: " + ", ".join(f"<#{c}>" for c in chans),
            mention_author=False,
        )

    # ----- slash commands (allowlist) -----

    @bot.tree.command(
        name="allow_here",
        description="(Admin) Allow this channel for bot commands in this server.",
    )
    ...
    @bot.tree.command(
        name="clear_allowed",
        description="(Admin) Clear all allowed-channel restrictions for this server.",
    )
    ...
    @bot.tree.command(
        name="list_allowed",
        description="(Admin) List allowed channels for this server.",
    )
    ...
    @bot.tree.command(
        name="setup",
        description="(Admin) Create & pin the bot config message in this channel.",
    )
    async def setup_slash(interaction: discord.Interaction):
        ...
        if not isinstance(channel, discord.TextChannel):
            return await interaction.response.send_message(
                "Please run this in a text channel.",
                ephemeral=True,
            )
    ...
    @bot.tree.command(
        name="reload-config",
        description="(Admin) Reload config from pinned message in this channel.",
    )
    async def reload_config_slash(interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.response.send_message(
                "I couldn't read pinned messages here. "
                "Do I have `Read Message History` (and optionally `Manage Messages`)?",
                ephemeral=True,
            )
